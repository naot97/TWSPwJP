...
for dataset in datasets:
  ...
  for json_name in list_json:
    if json_name >= 'input_20_2_3_5.json':
      ...
      #   for t in range(m[j]):
      #     for i in range(n):
      #       contrainst5.append(b[j][t] <= s[i][j][t])
      #       contrainst5.append(s[i][j][t] <= (b[j][t+1] - y[i][j][t]) )
      ...
      # contrainst = contrainst0 + contrainst1 + contrainst2 + contrainst3 + contrainst5  + contrainst6 
      ...
      begin = time.time()
      result = prob.solve('CPLEX',cplex_params= {'timelimit': 3600}, verbose=False)
      # result = 0
      processing_time = int(time.time() - begin)
      print(json_name, result, processing_time)
      data = {}
      data['name'] = json_name
      data['n'] = n
      data['k'] = k
      data['LB'] = LB
      data['C_max'] = result
      data['t'] = processing_time
      result_list.append(data)



      df = pd.DataFrame(result_list)
      df.to_excel('table11.xlsx')
